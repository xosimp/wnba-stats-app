...
async function scrapeNBATeamOpponentStats() {
  let browser;
  
  try {
    console.log('🏀 Starting NBA Team Opponent Stats Scraper for 2025 season...');
    console.log('📊 Source: Basketball Reference NBA Team Opponent Statistics');
    ...
    const tableExists = await page.evaluate(() => {
      let table = document.querySelector('#per_game-opponent');
      if (!table) {
    ...
    if (!tableExists) {
      throw new Error('❌ NBA team opponent stats table not found');
    }
    
    console.log('🔍 Looking for NBA team opponent stats table...');
    ...
    if (!tableSelector) {
      throw new Error('❌ Could not find NBA team opponent stats table');
    }
    
    console.log('📊 Extracting NBA team opponent stats...');
    ...
    const teamStats = await page.evaluate(() => {
      ...
      for (const row of rows) {
        try {
          ...
          stats.push(teamStat);
          console.log(`✅ Extracted opponent stats for: ${teamName} (${teamAbbreviation})`);
    ...
    console.log(`✅ Successfully extracted ${teamStats.length} NBA team opponent stats`);
    
    if (teamStats.length === 0) {
      console.log('⚠️  No NBA team opponent stats found to save');
      return;
    }
    
    // Save to database
    if (teamStats.length > 0) {
      console.log('💾 Saving NBA team opponent stats to database...');
      ...
      console.log(`✅ Successfully saved ${teamStats.length} NBA team opponent stats to database`);
      
      // Show sample of saved data
      console.log('\n📊 Sample NBA team opponent stats:');
      teamStats.slice(0, 3).forEach(team => {
        console.log(`   ${team.team_name} (${team.team_abbreviation}): Opponents ${team.points} PPG, ${team.total_rebounds} RPG, ${team.assists} APG`);
      });
    }
    
  } catch (error) {
    console.error('💥 NBA team opponent stats scraping failed:', error);
    throw error;
  } finally {
    if (browser) {
      await browser.close();
...
// Run the scraper
scrapeNBATeamOpponentStats()
  .then(() => {
    console.log('🎉 NBA team opponent stats scraping completed successfully!');
    process.exit(0);
  })
  .catch((error) => {
    console.error('💥 NBA team opponent stats scraping failed:', error);
