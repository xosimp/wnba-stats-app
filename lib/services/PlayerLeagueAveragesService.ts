 ...
 */
export class PlayerLeagueAveragesService {
  private supabase;
  private static instance: PlayerLeagueAveragesService;
  
  // Dynamic player-level league averages (calculated from database)
  private static PLAYER_LEAGUE_AVERAGES = {
    points: 13.2,
    rebounds: 5.4,
    assists: 3.3,
    turnovers: 2.3,
  ...
  private static TOP_1_PERCENT_THRESHOLDS = {
    points: 21.0,
    rebounds: 9.0,
    assists: 5.5,
    turnovers: 3.8,
  ...
  private static BOTTOM_1_PERCENT_THRESHOLDS = {
    points: 4.2,
    rebounds: 1.8,
    assists: 0.8,
    turnovers: 0.5,
   ...
   */
  static getInstance(supabaseUrl?: string, supabaseKey?: string): PlayerLeagueAveragesService {
  ...
  getLeagueAverage(statType: string): number {
    return PlayerLeagueAveragesService.PLAYER_LEAGUE_AVERAGES[statType as keyof typeof PlayerLeagueAveragesService.PLAYER_LEAGUE_AVERAGES] || 0;
  }

  /**
   * Get top 1% threshold for a specific stat type
   */
  getTop1PercentThreshold(statType: string): number {
    return PlayerLeagueAveragesService.TOP_1_PERCENT_THRESHOLDS[statType as keyof typeof PlayerLeagueAveragesService.TOP_1_PERCENT_THRESHOLDS] || 0;
  }

  /**
   * Get bottom 1% threshold for a specific stat type
   */
  getBottom1PercentThreshold(statType: string): number {
    return PlayerLeagueAveragesService.BOTTOM_1_PERCENT_THRESHOLDS[statType as keyof typeof PlayerLeagueAveragesService.BOTTOM_1_PERCENT_THRESHOLDS] || 0;
  }
  ...
  async updateLeagueAverages(season: string = '2025'): Promise<void> {
    try {
      ...
      console.log(`📊 Calculating averages from player season stats...`);
      ...
      // First, get player season averages instead of individual game values
      const { data: seasonStats, error: seasonError } = await this.supabase
        .from('player_season_stats')
        .select('*')
        .eq('season', '2025')
        .gt('games_played', 5); // Only include players who played at least 5 games
      ...
      if (!seasonStats || seasonStats.length === 0) {
        console.log('⚠️ No season stats found for league average calculation');
        return;
      }

      console.log(`📊 Calculating averages from ${seasonStats.length} player season averages...`);

      for (const statType of statTypes) {
        const validValues = seasonStats
          .map((player: any) => {
            switch (statType) {
              case 'points': return player.avg_points;
              case 'rebounds': return player.avg_rebounds;
              case 'assists': return player.avg_assists;
              case 'turnovers': return player.avg_turnovers;
              case 'steals': return player.avg_steals;
              case 'blocks': return player.avg_blocks;
              case 'minutes': return player.avg_minutes;
              default: return null;
            }
          })
          .filter((value: any) => typeof value === 'number' && !isNaN(value) && value >= 0);
        ...
        if (validValues.length > 0) {
          ...
          console.log(`📈 League average for ${statType}: ${leagueAverages[statType]} (from ${validValues.length} players)`);
        }
      }

      // Calculate percentiles for top/bottom 1% thresholds
      const top1Thresholds: Record<string, number> = {};
      const bottom1Thresholds: Record<string, number> = {};
   ...
   */
  private async saveLeagueAveragesToDatabase(
   ...
   */
  async refreshLeagueAverages(season: string = '2025'): Promise<void> {
