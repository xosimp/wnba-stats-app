...
export function getOrderedGames(arr: GameLogEntry[]): GameLogEntry[] {
  console.log('getOrderedGames called with:', { gamesLength: arr?.length || 0 });
  try {
    if (!arr || arr.length === 0) {
      console.log('getOrderedGames returning empty array');
      return [];
    }
    ...
    console.log('getOrderedGames result length:', sorted.length);
    return sorted;
  } catch (error) {
    console.error('Error in getOrderedGames:', error);
    return [];
  }
}

export function calculateOverPercentage(arr: GameLogEntry[], count: number, finalBookLine: number): number {
  console.log('calculateOverPercentage called with:', { gamesLength: arr?.length || 0, count, finalBookLine });
  try {
    if (!arr || arr.length === 0 || count === 0 || !finalBookLine) {
      console.log('calculateOverPercentage returning 0 due to invalid input');
      return 0;
    }
    ...
    if (recentGames.length === 0) {
      console.log('calculateOverPercentage returning 0 due to no valid games');
      return 0;
    }
    ...
    console.log('calculateOverPercentage result:', { overCount, recentGamesLength: recentGames.length, percentage });
    return Number.isFinite(percentage) ? percentage : 0;
  } catch (error) {
    console.error('Error in calculateOverPercentage:', error);
    return 0;
  }
}

export function getBarStyles(index: number, games: GameLogEntry[], selectedPeriod: string) {
  console.log('getBarStyles called with:', { index, gamesLength: games?.length || 0, selectedPeriod });
  try {
  ...
  return {
    ...
    console.error('Error in getBarStyles:', error);
    return { width: 32, marginLeft: 0, marginRight: 0 };
  }
}

export function getBookLinePosition(value: number, selectedPeriod: string) {
  console.log('getBookLinePosition called with:', { value, selectedPeriod });
  try {
    ...
    console.log('getBookLinePosition result:', `${position}px`);
    return `${position}px`;
  } catch (error) {
    console.error('Error in getBookLinePosition:', error);
    return '0px';
  }
}
